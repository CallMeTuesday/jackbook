...
interface NavbarProps {
  /** Home page passes this to keep search in sync with local state */
  search?: string
...
export function Navbar({ search: externalSearch, onSearchChange }: NavbarProps) {
  ...
  // Local input state, synced from external when on home page
  const [inputValue, setInputValue] = useState(externalSearch ?? '')
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  // Keep input in sync when home page clears search externally
  useEffect(() => {
  ...
  function handleInput(value: string) {
    ...
    if (isHome && onSearchChange) {
      // Home page: instant local filtering
      onSearchChange(value)
    } else {
      // Other pages: debounce → navigate home with query, preserving view param
      ...
      {status === 'loading' ? (
        ...
        showDevForm ? (
          <form onSubmit={handleDevLogin} className="flex items-center gap-1.5">
            ...
            />
            <Button type="submit" size="sm" disabled={signingIn || !devUsername.trim()}
          ...
          <Button onClick={() => setShowDevForm(true)} variant="ghost" size="sm"
  ...
  return (
    <nav className="border-b border-zinc-800 bg-zinc-950/90 backdrop-blur-sm sticky top-0 z-50">
      <div className="px-4">
        ...
        {/* Desktop: 3-col grid with equal outer cols → search is truly page-centered */}
